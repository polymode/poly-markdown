...
(ert-deftest infra/poly-markdown/pm-map-over-modes-regions ()
  (let ((pm-use-cache t)
        (ref-regions '((markdown-mode nil nil 1 391)
                       (fortran-mode body body 391 872)
                       (markdown-mode tail nil 872 1043)
                       (perl-mode body body 1043 1597)
                       (markdown-mode tail nil 1597 1849)
                       (emacs-lisp-mode body body 1849 2239)
                       (markdown-mode tail nil 2239 2368)
                       (pascal-mode body body 2368 2587)
                       (markdown-mode tail nil 2587 2897)
                       (python-mode body body 2897 3502)
                       (markdown-mode tail nil 3502 3587)
                       (emacs-lisp-mode body body 3587 4377)
                       (markdown-mode tail nil 4377 4469)
                       (sql-mode body body 4469 4532)
                       (markdown-mode tail nil 4532 4546)))
        acc)

    (cl-labels ((sub-ref (beg &optional end)
                  (setq end (or end (point-max)))
                  (let ((acc ref-regions)
                        out)
                    (while (and acc (< (nth 3 (car acc)) beg))
                      (pop acc))
                    (while (and acc (< (nth 3 (car acc)) end))
                      (push (pop acc) out))
                    (reverse out)))
                (req (beg end)
                  (let ((span (pm-innermost-span beg)))
                    (push (list (pm-span-mode span) (car span) (pm-true-span-type span)
                                beg end)
                          acc)))
                (run (beg end)
                  (setq acc nil)
                  (pm-map-over-modes #'req beg end)
                  (sort acc (lambda (x y) (< (nth 3 x) (nth 3 y))))))
      
      ;; ;; exact span
      ;; (pm-test-run-on-file poly-markdown-mode "markdown.md"
      ;;   (let ((ref '((fortran-mode body body 391 872))))
      ;;     (should (equal ref (run 391 872)))
      ;;     (should (equal ref (run 391 872)))
      ;;     (let ((pm-use-cache nil))
      ;;       (should (equal ref (run 391 872))))))

      ;; (pm-test-run-on-file poly-markdown-mode "markdown.md"
      ;;   (let ((ref '((markdown-mode head nil 380 391))))
      ;;     (should (equal ref (run 380 391)))
      ;;     (should (equal ref (run 380 391)))
      ;;     (let ((pm-use-cache nil))
      ;;       (should (equal ref (run 380 391))))))

      ;; (pm-test-run-on-file poly-markdown-mode "markdown.md"
      ;;   (let ((ref '((markdown-mode tail nil 872 875))))
      ;;     (let ((pm-use-cache nil))
      ;;       (should (equal ref (run 872 875))))
      ;;     (should (equal ref (run 872 875)))))

      ;; multi spans
      (pm-test-run-on-file poly-markdown-mode "markdown.md"
        (let ((pm-use-cache nil))
          (should (equal ref-regions (run 1 (point-max)))))
        (should (equal ref-regions (run 1 (point-max)))))


      (pm-test-run-on-file poly-markdown-mode "markdown.md"
        (should (equal ref-regions (run 200 (point-max))))
        (let ((pm-use-cache nil))
          (should (equal ref-regions (run 200 (point-max)))))
        (should (equal ref-regions (run 200 (point-max)))))
      ...
      (pm-test-run-on-file poly-markdown-mode "markdown.md"
        (let ((ref (sub-ref 872)))
          (let ((pm-use-cache nil))
            (should (equal ref (run 872 (point-max)))))
          (should (equal ref (run 872 (point-max))))))
      
      (pm-test-run-on-file poly-markdown-mode "markdown.md"
        (let ((ref (sub-ref 1843)))
          (should (equal ref (run 2221 (point-max))))
          (let ((pm-use-cache nil))
            (should (equal ref (run 2221 (point-max)))))
          (should (equal ref (run 2221 (point-max))))))

      (pm-test-run-on-file poly-markdown-mode "markdown.md"
        (let ((ref (sub-ref 2239)))
          (let ((pm-use-cache nil))
            (should (equal ref (run 2239 (point-max)))))
          (should (equal ref (run 2239 (point-max))))))

      (pm-test-run-on-file poly-markdown-mode "markdown.md"
        (let ((ref '((markdown-mode tail nil 2239 2358))))
          (should (equal ref (run 2239 2256)))
          (let ((pm-use-cache nil))
            (should (equal ref (run 2239 2256))))))

      (pm-test-run-on-file poly-markdown-mode "markdown.md"
        (let ((ref '((markdown-mode tail nil 2239 2358))))
          (should (equal ref (run 2239 2347)))
          (let ((pm-use-cache nil))
            (should (equal ref (run 2239 2347))))))

      (pm-test-run-on-file poly-markdown-mode "markdown.md"
        (let ((ref (sub-ref 4532)))
          (should (equal ref (run 4533 (point-max))))
          (let ((pm-use-cache nil))
            (should (equal ref (run 4533 (point-max)))))))

      (pm-test-run-on-file poly-markdown-mode "markdown.md"
        (let ((ref '((markdown-mode tail nil 4532 4546))))
          (should (equal ref (run 4532 (point-max))))
          (let ((pm-use-cache nil))
            (should (equal ref (run 4532 (point-max)))))))

      (pm-test-run-on-file poly-markdown-mode "markdown.md"
        (let ((ref (cons '(emacs-lisp-mode body body 3587 4377)
                         (sub-ref 4369))))
          (should (equal ref (run 4369 (point-max))))
          (let ((pm-use-cache nil))
            (should (equal ref (run 4369 (point-max)))))))
